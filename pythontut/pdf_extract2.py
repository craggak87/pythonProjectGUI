...
def right_arrow(all_images, current_image, what_text):
    if imge_idx[-1] < len(all_images) -1:
        new_idx = imge_idx[-1] + 1
        imge_idx.pop()
        imge_idx.append(new_idx)
        if displayed_img:
            displayed_img[-1].grid_forget()
            displayed_img.pop()
        new_img = all_images[imge_idx[-1]]
        current_image = display_images(new_img)
        displayed_img.append(current_image)
        what_text.set("image" + str(imge_idx[-1]+1) + "out of "+ str(len(all_images)))
    elif imge_idx == len(all_images) -1:
        print("index out of range")
        if displayed_img:
            displayed_img[-1].grid_forget()
            displayed_img.pop()

def left_arrow(all_images, current_image, what_text):
    if imge_idx[-1] >=1:
        new_idx = imge_idx[-1] - 1
        imge_idx.pop()
        imge_idx.append(new_idx)
...
def open_file():
    ...
    if file:
        ...
        save_btn = Button(root, text="Save image", command=lambda:save_image(all_images[imge_idx[-1]]), font=("shanti", 10), height=1, width=15)
